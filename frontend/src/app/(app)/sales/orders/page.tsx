    ...
    <>
      ...
      <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-[var(--surface)] p-3 rounded-lg border border-[var(--border)] shadow-sm">
        <Link href="/sales/orders/new">
          <Button variant="primary" className="font-semibold"><Plus className="h-4 w-4 mr-2" /> New Sales Order</Button>
        </Link>
      ...
      <div className="mt-4">
        {view === "list" ? (
          <OrderTable orders={filteredData} type="sales" />
        ) : (
          <OrderKanban orders={filteredData} type="sales" />
        )}
      </div>
    </>
  );
